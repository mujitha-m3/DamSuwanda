  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        await audio.load(local ? { localUri: local } : { url: pirith.audioUrl });
      } catch {}
      setLoading(false);
    })();
...
const styles = StyleSheet.create({
  ...
  sliderContainer: { width: '100%', height: 20, justifyContent: 'center' },
  sliderTrack: { width: '100%', height: 3, backgroundColor: Colors.divider, borderRadius: 2 },
  sliderFill: { height: 3, backgroundColor: Colors.primaryGold, borderRadius: 2 },
  sliderThumb: { position: 'absolute', width: 14, height: 14, borderRadius: 7, backgroundColor: Colors.primaryGold, marginLeft: -7, top: 3 },
